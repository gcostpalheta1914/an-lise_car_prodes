...
if st.button("🚀 Processar Cruzamento Espacial"):
    if f_c and f_p:
        with st.spinner("Extraindo e buscando arquivos..."):
            try:
                ...
                # 1. Encontrar o PRODES (varre todas as subpastas)
                p_path = None
                for r, _, fs in os.walk("tmp/p"):
                    for file in fs:
                        if file.endswith(".shp"):
                            p_path = os.path.join(r, file)
                            break
                
                # Leitura otimizada
                g_p = gpd.read_file(p_path, engine='pyogrio').to_crs("EPSG:31982")
                
                # 2. Encontrar TODOS os CARs (varre todas as subpastas)
                res = []
                for r, _, fs in os.walk("tmp/c"):
                    for file in fs:
                        if file.endswith(".shp"):
                            ...
                            if not inter.empty:
                                # Ajuste aqui conforme os nomes das suas colunas
